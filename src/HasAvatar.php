    ...
    public function updateAvatar(UploadedFile $avatar)
    {
        tap($this->avatar_path, function ($previous) use ($avatar) {
            if (config('aether.use_fancy_avatar_paths')) {
                ...
                $xl = Image::make($avatar)->fit(
                    config('aether.xl_avatar_size', '384'),
                    config('aether.xl_avatar_size', '384')
                )->encode();
                $lg = Image::make($avatar)->fit(
                    config('aether.lg_avatar_size', '384'),
                    config('aether.lg_avatar_size', '192')
                )->encode();
                $md = Image::make($avatar)->fit(
                    config('aether.md_avatar_size', '384'),
                    config('aether.md_avatar_size', '96')
                )->encode();
                $sm = Image::make($avatar)->fit(
                    config('aether.sm_avatar_size', '384'),
                    config('aether.sm_avatar_size', '48')
                )->encode();

                Storage::disk($this->avatarDisk())->put(
                    'users/'.$this->id.'/xl_avatar.'.$avatar->getClientOriginalExtension(),
                    $xl
                );
                Storage::disk($this->avatarDisk())->put(
                    'users/'.$this->id.'/l_avatar.'.$avatar->getClientOriginalExtension(),
                    $lg
                );
                Storage::disk($this->avatarDisk())->put(
                    'users/'.$this->id.'/m_avatar.'.$avatar->getClientOriginalExtension(),
                    $md
                );
                Storage::disk($this->avatarDisk())->put(
                    'users/'.$this->id.'/s_avatar.'.$avatar->getClientOriginalExtension(),
                    $sm
                );
